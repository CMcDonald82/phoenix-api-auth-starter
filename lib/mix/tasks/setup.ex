defmodule Mix.Tasks.Setup do
  ...
  @shortdoc "Sets up a new clean-slate project using the phoenix-api-auth-starter project as a base template"

  @doc """
  The main function that runs all the necessary functions to setup the new app from the phoenix-api-auth-starter project 
  """
  ...
  """
  def config do
    Application.get_env(:phoenix_api_auth_starter, Mix.Tasks.Setup)
  end
  ...
  def rename_app(name, otp_name) do
    ...
    Rename.run(
      {"PhoenixApiAuthStarter", name},
      {"phoenix_api_auth_starter", otp_name}
    )
    :ok
  rescue 
